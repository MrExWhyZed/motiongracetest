...
function ShowcaseCard({
  ...
  /* Stagger the content reveal slightly after expand */
  useEffect(() => {
    if (isHovered) {
      revealTimerRef.current = setTimeout(() => setRevealed(true), 120);
    } else {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    /* Magnetic pull — card body follows cursor softly */
    const dx = (nx - 0.5) * 18;
    const dy = (ny - 0.5) * 10;
  ...
  const tiltX = isHovered ? (mouse.y - 0.5) * -14 : 0;
  const tiltY = isHovered ? (mouse.x - 0.5) * 14 : 0;
  ...
  return (
    <div
      ...
      style={{
        ...
        width: isHovered
          ? 'clamp(480px, 46vw, 620px)'
          : 'clamp(260px, 26vw, 340px)',
        transform: `translateY(${depthY}px) translate(${isHovered ? magnetOffset.x * 0.4 : 0}px, ${isHovered ? magnetOffset.y * 0.4 : 0}px)`,
        transition: 'width 0.7s cubic-bezier(0.16,1,0.3,1), transform 0.6s cubic-bezier(0.16,1,0.3,1)',
    ...
    >
      <div
        ...
        style={{
          ...
          transform: `perspective(900px) rotateX(${tiltX}deg) rotateY(${tiltY}deg) translate(${isHovered ? magnetOffset.x * 0.6 : 0}px, ${isHovered ? magnetOffset.y * 0.6 : 0}px)`,
          transition: isHovered
            ? 'transform 0.12s ease, height 0.7s cubic-bezier(0.16,1,0.3,1), border-color 0.4s ease, box-shadow 0.4s ease'
            : 'transform 0.8s cubic-bezier(0.16,1,0.3,1), height 0.7s cubic-bezier(0.16,1,0.3,1), border-color 0.5s ease, box-shadow 0.5s ease',
          boxShadow: isHovered
      ...
      >
        ...
        <img
          ...
          style={{
            ...
            objectFit: 'cover',
            transform: isHovered
              ? `scale(1.12) translate(${(mouse.x - 0.5) * -8}px, ${(mouse.y - 0.5) * -8}px)`
              : 'scale(1)',
            transition: isHovered ? 'transform 0.4s ease' : 'transform 2s cubic-bezier(0.16,1,0.3,1)',
        ...
        <div style={{
          position: 'absolute', inset: 0,
          background: `radial-gradient(circle at ${mouse.x * 100}% ${mouse.y * 100}%, rgba(${item.accentRgb},0.18) 0%, transparent 55%)`,
          opacity: isHovered ? 1 : 0,
          transition: isHovered ? 'opacity 0.3s ease' : 'opacity 0.6s ease',
          mixBlendMode: 'screen',
        }} />
        ...
        <div style={{
          ...
          left: `calc(${mouse.x * 100}% - 5px)`,
          top: `calc(${mouse.y * 100}% - 5px)`,
          width: '10px', height: '10px', borderRadius: '50%',
          background: item.accent,
          boxShadow: `0 0 16px 4px rgba(${item.accentRgb},0.5)`,
          opacity: isHovered ? 0.9 : 0,
          transition: isHovered ? 'left 0.08s ease, top 0.08s ease, opacity 0.3s ease' : 'opacity 0.4s ease',
          mixBlendMode: 'screen',
        }} />
        ...
        <div style={{
          ...
          left: `calc(${mouse.x * 100}% - 22px)`,
          top: `calc(${mouse.y * 100}% - 22px)`,
          width: '44px', height: '44px', borderRadius: '50%',
          border: `1px solid rgba(${item.accentRgb},0.35)`,
          opacity: isHovered ? 1 : 0,
          transition: isHovered ? 'left 0.2s ease, top 0.2s ease, opacity 0.3s ease' : 'opacity 0.4s ease',
        }} />
      </div>
    </div>
  );
...
export default function ShowcaseSection() {
  ...
  const pauseVelocityRef = useRef(0);  // momentum decay when pausing
  ...
  /* Auto-scroll with pause momentum */
  useEffect(() => {
    ...
    const loop = (time: number) => {
      ...
      if (!isPausedRef.current) {
        /* Smoothly restore speed after pause */
        pauseVelocityRef.current = Math.min(pauseVelocityRef.current + 0.018, 1);
        targetXRef.current = (targetXRef.current + BASE_SPEED * (delta / 16) * pauseVelocityRef.current) % trackWidth;
      } else {
        /* Momentum decay when paused */
        pauseVelocityRef.current = Math.max(pauseVelocityRef.current - 0.04, 0);
        targetXRef.current = (targetXRef.current + BASE_SPEED * (delta / 16) * pauseVelocityRef.current) % trackWidth;
      }
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        /* Extra vertical room for card expansion */
        ...
        >
          {allItems.map((item, i) => {
            ...
            /* neighbor: same-id siblings in duplicated array, or adjacent */
      ...
      <style>{`
        @keyframes streak {
          0%, 100% { opacity: 0.4; transform: translateX(0) rotate(var(--rot, -7deg)); }
          50%       { opacity: 1;   transform: translateX(2%) rotate(var(--rot, -7deg)); }
        }
        @keyframes dot-pulse {
